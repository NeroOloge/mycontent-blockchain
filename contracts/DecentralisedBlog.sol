...
contract DecentralisedBlog {
  ...
  event ProfileUpdated(address indexed user, string username, string bio, string imageCID);
  ...
  function addComment(uint256 postId, string calldata cid) external {
    require(posts[postId].exists, "Post doesn't exist");
    ...
    emit CommentAdded(nextCommentId, postId, msg.sender, cid);
    nextCommentId++;
  }
  ...
  function tipAuthor(uint256 postId) external payable {
    ...
    emit Tipped(msg.sender, author, msg.value);
  }

  function updateProfile(string calldata username, string calldata bio, string calldata imageCID) 
  external {
    ...
    emit ProfileUpdated(msg.sender, username, bio, imageCID);
  }
